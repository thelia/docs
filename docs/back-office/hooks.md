...
Hooks are extension points in templates where modules inject their own markup, add features, and change the appearance of a page — without editing the template itself.
...
In Thelia 3, hooks remain the way modules extend the **back-office** UI. The back-office is now the **`default-twig` bundle** (Bootstrap 5 / Twig / Stimulus), so hook content is consumed through **Twig functions** (`hook()`, `hook_block()`, …), not the Smarty `{hook}` tag.
...
The back-office exposes two families of functions: single hooks that return a concatenated string, and block hooks that return iterable fragments.
...
a warning instead of breaking the page — useful while a third-party module still ships only
...
- `hook_block(name, params)` returns a `Thelia\Core\Hook\FragmentBag` (iterable). Each fragment exposes its keys as Twig properties (`block.title`, `block.content`, `block.href`, …).
...
A `BaseHook` subclass is wired automatically — no XML and no manual service tag. Declaring `getSubscribedHooks()` is enough for the listener methods to be registered against the matching hook events.
:::
...
A module listening on the old name keeps contributing — no change required.
...
The `default-twig` back-office emits its hooks following stable conventions, so a module can rely on a predictable set of extension points instead of memorising a per-screen inventory.
